...
def weil_poly_from_point_count(l, q):
    P.<T> = QQ[]
    Q.<t> = PowerSeriesRing(QQ)
    g = len(l)
    u = sum(l[i-1]*t^i/i for i in range(1,g+1))
    v = exp(u)*(1-t)*(1-q*t)
    l2 = [v[i] for i in range(g+1)]
    for i in range(1, g+1):
        l2.append(q^i*v[g-i])
    return P(l2).reverse()

def weil_poly_prefix_from_traces(P, q, l):
    Q.<t> = PowerSeriesRing(QQ)
    u = sum(-l[i-1]*t^i/i for i in range(1,len(l)+1))
    v = exp(u)
    l2 = v.polynomial().list()[:len(l)+1]
    try:
        return [ZZ(i) for i in l2]
    except TypeError:
        raise ValueError("Traces cannot come from a Weil polynomial")

# Identify all Weil polynomials with specified initial Frobenius traces.
def weil_polys_from_traces(P, d, q, l):
    l2 = weil_poly_prefix_from_traces(P, q, l)
    if len(l2) > d//2 + 1:
        l2 = l2[:d//2 + 1]
...
# the Deuring-Shafarevich formula; do not enable this unless you know what you are doing!
...
def _nojac_serre(pol, q, alert=False, castelnuovo=False, blocklist=None):
    ...
    for i in range(2^(n-1)-1):
        ...
        if res == 1:
            return True
        if res == 2 and castelnuovo: # Use with caution!
